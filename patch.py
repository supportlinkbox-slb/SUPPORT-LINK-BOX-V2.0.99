...
# 1. Add inviteTokenHash state
content = re.sub(
    r"const \[inviteToken, setInviteToken\] = useState\(''\);",
    r"const [inviteToken, setInviteToken] = useState('');\n  const [inviteTokenHash, setInviteTokenHash] = useState('');",
    content
)

# 2. Replace handleVerifyInvite
handleVerify = """  const handleVerifyInvite = async () => {
    ...
    try {
      ...
      setSuccessNotice({
        ...
        message: 'টোকেন সঠিক। দয়া করে যে ইমেইল দিয়ে ইনভাইট করা হয়েছে সেটি ব্যবহার করে রেজিস্ট্রেশন করুন বা লগইন করুন।'
      });
      setMode('REGISTER');
    } catch (err: any) {
...
content = re.sub(
    r"const handleVerifyInvite = async \(\) => \{.*?(?=const handleSubmit = async)/s",
    handleVerify + "\n\n  ",
    content,
    flags=re.DOTALL
)

# 3. Replace the INVITE_TOKEN logic inside handleSubmit
content = re.sub(
    r"if \(isInvite\) \{.*?return;\n    \}",
    """if (isInvite) {
...
# 4. Pass tokenHash to register
content = re.sub(
    r"facebookIdentityType: fbValidation\?\.identityType,",
